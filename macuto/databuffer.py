...
class HdfDataBuffer(object):
    ...
    def create_dataset(self, ds_name, data, dtype=None):
        """
...
class NumpyHDFStore(HDFStore):
    ...
    def _fill_missing_values(df, range_values, fill_value=0, fill_method=None):
        ...
        """
        idx_colnames  = tuple(df.index.names)

        idx_colranges = [range_values[x] for x in idx_colnames]

        fullindex = pd.Index([p for p in product(*idx_colranges)], name=idx_colnames)
    ...
    def put(self, key, value, format=None, append=False, **kwargs):
        ...
        if not isinstance(value, np.ndarray):
            ...
            self._handle.createArray(group, ds_name, value)

            self._handle.flush()

    def put_df_as_ndarray(self, key, df, range_values, fill_value=0, fill_method=None):
        """
        ...
        for idx in df:
            ...
            vals_colranges = [range_values[x] for x in vals.index.names]
            nu_shape = [len(x) for x in vals_colranges]

            self.put(key, np.reshape(vals.values, tuple(nu_shape)), ds_name=ds_name, append=True)