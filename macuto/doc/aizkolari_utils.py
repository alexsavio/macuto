...
setup_logger(verbosity=1, logfname=None)
...
#----------------------------------------------------------------
def sys_call (comm_line):
  subprocess.call(comm_line.split(' '))
...
def shelve_global_vars (outfpath, varlist):
   mashelf = shelve.open(outfpath, 'n', writeback=True)
...
def shelve_varlist (outfpath, varnames, varlist):
    ...
    for i in np.arange(len(varnames)):
       try:
           ...
           log.error('ERROR shelving: {0}'.format(varnames[i]))
           print sys.exc_info()
...
def get_nii_data(niipath):
    ...
    try:
        ...
        log.debug ('get_nii_data: Error on reading file ' + niipath)
