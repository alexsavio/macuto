...
#==============================================================================
def subplot_this(plt, ax, xlabel, ylabel, prefs_thrs, yvals, yvariances, yrange, c,  clor='k', mark='o', sty='_', show_legend=True):
    ...
    for j in list(ParameterGrid(preprocess_grid)):
        ...
        for prefs_m in prefs_methods:
            ...
            for prefs_thr in prefs_thrs:
                ...
                fname = 'tanimot_distance_matrix_' + prefs_m + '_' + str(prefs_thr) + '.png'
...
for j in list(ParameterGrid(preprocess_grid)):
    cs.do_atlasquery(j['temporal_filtering'], j['global_nuis_correction'], overwrite, j['atlas'], aq_thresh)
'''
def do_atlasquery (temporal_filtering=True, global_nuis_correction=True, overwrite=False, atlas='MNI Structural Atlas', aq_thresh=0.95):
    ...
    for j in list(ParameterGrid(param_grid)):
        ...
        if atlas == 'MNI Structural Atlas':
            out_autoaq_txt += '_mni_struct'
        elif atlas == 'Harvard-Oxford Cortical Structural Atlas':
            out_autoaq_txt += '_harvox_cort'
        elif atlas == 'Harvard-Oxford Subcortical Structural Atlas':
            out_autoaq_txt += '_harvox_subcort'

        out_autoaq_txt += '_autoaq_presels.txt' #presels is the same for all classifiers
