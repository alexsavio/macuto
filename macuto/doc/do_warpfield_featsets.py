...
#fsmethods  = ['none', 'stats', 'hist3d']
fsmethods  = ['hist3d']
featstypes = ['warpfield']
...
for feats in featstypes:
    ...
    for fsmethod in fsmethods:
        ...
        do_hist3d_dataset  (datadir, subjlstf, maskf, smoothmm, bins, feats, outdir, otype)
...
def set_parser():
    ...
    feats        = ['jacs','smoothmodgm','modulatedgm', 'geodan', 'norms', 'trace', 'warpfield']
    outs         = ['.npy','.mat']

    parser = argparse.ArgumentParser(description='OASIS AAL classification experiment.')
    parser.add_argument('-s', '--subjlstf',  dest='subjlstf', default='', required=True,   help='list file with the subjects for the analysis. Each line: <class_label>,<subject_file_3D_warpfield>')
    ...
    parser.add_argument('-m', '--mask',      dest='maskf',    default='', required=False,  help='path to a mask file for all subjects')
    ...
    try:
        ...
        for s in f:
            ...
            if not os.path.isabs(subjf):
                subjs.append (os.path.join(datadir,subjf))
            else:
                subjs.append (subjf)
...
#-------------------------------------------------------------------------------
def calculate_hist3d_data (subjsf, datadir, msk, bins, smoothmm=0):
    '''
    subjf = os.path.join(wd, 'warpfield/control.F.18.00.OAS1_0076_MR1_mpr_n4_anon_111_t88_masked_gfc_warpfield.nii.gz')
    ...
    H, [bx, by, bz] = np.histogramdd((x,y,z), bins=(bins,bins,bins), normed=True)
    ...
    #loading data
    au.log.info ('Loading data...')
    X = np.zeros((nsubjs, pow(bins,3)), dtype=dtype)
    for f in np.arange(nsubjs):
        imf = subjs[f]
        ...
        if imgsiz[3] == 3:
            if (smoothmm > 0):
                ...
                h, [bx, by, bz] = np.histogramdd((vx,vy,vz), bins=(bins,bins,bins), normed=True)
...
#-------------------------------------------------------------------------------
def create_raw_feature_sets (data, y, outdir, outbasename, otype):
    n_subjs = data.shape[0]
    ...
    #save file
    save_feats_file (data, otype, outfname)
...
#-------------------------------------------------------------------------------
def do_hist3d_dataset  (datadir, subjlstf, maskf, smoothmm, bins, feats, outdir, otype):
    ...
    #load all data
    data, y, scores = calculate_hist3d_data (subjlstf, datadir, msk, bins, smoothmm)
    ...
    #create space for all features and read from subjects
    create_raw_feature_sets (data, y, outdir, outbasename, otype)
...
def main():
    ...
    if fsmethod != 'warpfield':
        return main_do (datadir, subjlstf, smoothmm, feats, outdir, roisdir, roilabsf, fsmethod, otype)
    else:
        return do_hist3d_dataset (datadir, subjlstf, maskf, smoothmm, bins, feats, outdir, otype)
