...
class LAM(object):
    ...
    (LHAM).
    #---------------------------------------------------------------------------
    Input:   X         : input pattern matrix [n_samples x n_variables]
             Y         : output pattern matrix [n_samples x m_variables]

    Output:  W         : dilative LAM [m_variables x n_variables]
             M         : erosive LAM [m_variables x n_variables]
...
class ILSIA(object):
    ...
    #---------------------------------------------------------------------------
    Input:   data   : column data matrix [n_samples x n_variables]
             alpha  : Chebyshev-best approximation tolerance threshold (>= 0). \
                      Default = 0.

    Output:  em     : set of induced endmembers [p x n_variables]
             cnt    : induced endmembers indexes vector [n_samples] with {0,1} \
...
class EIHA(object):
    ...
    ----------------------------------------------------------------------------
    Input:   data      : column data matrix [nsamples x nvariables]
             alpha     : perturbation tolerance. Default = 2

    Output:  em        : set of induced endmembers [p x nvariables]
             cnt       : induced endmembers indexes vector [nsamples] with {0,1} values, where '1' indicates that the corresponding sample has been identified as an endmember.
...
class WM(object):
    ...
    Output:  em        : set of induced endmembers [p x nchannels]
...
class HFC(object):
    ...
    ----------------------------------------------------------------------------
    Input:   data      : column data matrix [nsamples x nvariables]
             far       : list of false alarm probabilities [1 x p].
...
class NFINDR(object):
    ...
    Input:   data      : column data matrix [nsamples x nvariables]
             p         : number of endmembers to be induced. 
                         If not provided it is calculated by HFC method with 
                         tol=10**-5
...
class CCA(object):
    """
     ...
     ---------------------------------------------------------------------------
     Input:   data      : column data matrix [nsamples x nvariables]
              p         : number of endmembers to be induced. If not provided it
                          is calculated by HFC method with tol=10^(-5).
              t         : tolerance for numerical errors. 
                          By default 10^(-6)

     Output:  em        : set of induced endmembers [nchannels x p]
...
class ATGP(object):
    ...
    ----------------------------------------------------------------------------
     ...
     Output:  E         : set of induced endmembers [p x nvariables]
              C         : induced endmembers indexes vector [nsamples] with 
                          {0,1} values, where '1' indicates that the 
